...
function LoginPopUp() {
    ...
    return (
        <>
          <button>
            <User className="cursor-pointer h-8 w-8" onClick={() => setActive(true)} />
          </button>
          <div className={`bg-overlay ${active ? "active" : ""}`} onClick={() => setActive(false)}> </div>
          <div className={`login-container ${active ? "active" : ""}`} onClick={() => setActive(false)}>
            <form onSubmit={(e) => e.preventDefault()} className="login-wrapper" onClick={(e) => e.stopPropagation()} >
